...
const (
	// gives the following constants incrementing numbers as values
	// higher number == higher operator precedent
	_ int = iota
	LOWEST
	EQUALS      // ==
	LESSGREATER // < or >
...
func (p *Parser) parseCallExpression(function ast.Expression) ast.Expression {
	...
	exp.Arguments = p.parseExpressionList(token.R_PAREN)
	return exp
}
...
func (p *Parser) parseBlockStatement() *ast.BlockStatement {
	...
	for !p.currTokenIs(token.R_BRACE) && !p.currTokenIs(token.EOF) {
		stmt := p.parseStatement()
		if stmt != nil {
